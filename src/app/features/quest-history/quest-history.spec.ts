...
import { QUEST_HISTORY_MOCK } from './quest-history.mocks';
...
function createFixture(
  versionSvcMock: Partial<QuestVersionService>,
  loggedIn = true,
): ComponentFixture<QuestHistory> {
  TestBed.configureTestingModule({
    imports: [QuestHistory],
    providers: [
      provideRouter([]),
      {
        provide: ActivatedRoute,
        useValue: { snapshot: { paramMap: convertToParamMap({ gameId: '1', questId: '1' }) } },
      },
      { provide: QuestVersionService, useValue: versionSvcMock },
      { provide: AuthService, useValue: makeAuth(loggedIn) },
      { provide: ToastService, useValue: TOAST_MOCK },
    ],
  });
  const fixture = TestBed.createComponent(QuestHistory);
...
describe('QuestHistory', () => {
  ...
  it('deve criar o componente', () => {
    const fixture = createFixture({ list: vi.fn(() => of(QUEST_HISTORY_MOCK)) });
    expect(fixture.componentInstance).toBeTruthy();
  });

  it('carrega as versões na inicialização', () => {
    const listSpy = vi.fn(() => of(QUEST_HISTORY_MOCK));
    const fixture = createFixture({ list: listSpy });
    const comp = fixture.componentInstance as any;
    expect(listSpy).toHaveBeenCalledWith('1');
    expect(comp.versions()).toHaveLength(QUEST_HISTORY_MOCK.length);
  ...
  it('current() aponta para a versão com status current', () => {
    const fixture = createFixture({ list: vi.fn(() => of(QUEST_HISTORY_MOCK)) });
    const comp = fixture.componentInstance as any;
    expect(comp.current()?.versionNumber).toBe(4);
  ...
  describe('revert()', () => {
    it('adiciona nova versão e exibe toast de sucesso ao reverter', () => {
      const newVersion = { ...QUEST_HISTORY_MOCK[1], versionNumber: 5, status: 'current' as const };
      const svcMock = {
        list: vi.fn(() => of(QUEST_HISTORY_MOCK)),
        revert: vi.fn(() => of(newVersion)),
      };
      const fixture = createFixture(svcMock);
      const comp = fixture.componentInstance as any;
      comp.revert(QUEST_HISTORY_MOCK[1]);
      expect(svcMock.revert).toHaveBeenCalledWith('1', QUEST_HISTORY_MOCK[1].versionNumber);
  ...
  describe('toggleVote()', () => {
    it('registra voto e exibe toast de aviso', () => {
      const updated = { ...QUEST_HISTORY_MOCK[0], revertVotes: 3, userHasVoted: true };
      const svcMock = {
        list: vi.fn(() => of(QUEST_HISTORY_MOCK)),
      ...
      const comp = fixture.componentInstance as any;
      comp.toggleVote();
      expect(svcMock.voteRevert).toHaveBeenCalledWith('1');
      expect(TOAST_MOCK.warning).toHaveBeenCalled();
      expect(comp.voting()).toBe(false);
    });

    it('remove voto quando userHasVoted=true', () => {
      const votedVersion = { ...QUEST_HISTORY_MOCK[0], userHasVoted: true };
      const versions = [votedVersion, ...QUEST_HISTORY_MOCK.slice(1)];
      ...
      const comp = fixture.componentInstance as any;
      comp.toggleVote();
      expect(svcMock.removeVoteRevert).toHaveBeenCalledWith('1');
      expect(comp.voting()).toBe(false);
    });

    it('não faz nada quando não há versão current', () => {
      const versions = QUEST_HISTORY_MOCK.map((v) => ({ ...v, status: 'active' as const }));
      const svcMock = {
        list: vi.fn(() => of(versions)),
  ...
  describe('votePercent()', () => {
    it('calcula a porcentagem corretamente', () => {
      const fixture = createFixture({ list: vi.fn(() => of(QUEST_HISTORY_MOCK)) });
      const comp = fixture.componentInstance as any;
      const v = { revertVotes: 3, revertVotesNeeded: 5 };
      expect(comp.votePercent(v)).toBe(60);
    });
  ...
  it('initials() retorna as duas primeiras letras em maiúsculo', () => {
    ...
    expect(comp.initials('aldric_s')).toBe('AL');
  });
});
